...
const CartItem = ({ onContinueShopping }) => {
  ...
  const handleRemove = (item) => {
    dispatch(removeItem({ name: item.name })); // Dispatch removeItem to delete the item from the cart
  };

  // Calculate total cost based on quantity for an item
  const calculateTotalCost = (item) => {
    const itemPrice = parseFloat(item.cost.substring(1)); // Extract numeric value from item's cost
